...
f=open('PRINCE_TR'+str(TR)+'_mat_'+str(mat)+'.tex','w')
...
f.write(r'\begin{scope}[yshift=' + str(20) + 'cm]' + '\n' + r'   \begin{scope}[xshift=' + str(
        -4) + 'cm]' + '\n')
f.write(r'\path (-1,-0.5) node {\tiny round $R' + str(- TR) + '$};' + '\n')
f.write(r'\draw[->,dashed] (-1,1.5) -- ++(1,0);' + '\n')
f.write(r'\draw[->] (2,1.5) -- ++(1,0);' + '\n')
f.write(r'\draw[thin] (2.5,1.5) -- node[above] {\tiny ${\texttt{SB}}^{-1}$} +(0,0);' + '\n')
f.write(r'\draw(1,0) node {$Z_{R' + str(- TR) + '}$};' + '\n')
for i in range(0, 4):
    for j in range(0, 4):
        f.write(r'  \draw[fill=' + Xcolor[4 * i + j] + '] (' + str(j / 2) + ',' + str(
...
for r in range(0, mat):
    ...
    f.write(r'\draw[->] (2,1.5) -- ++(1,0);' + '\n')
    f.write(r'\draw[thin] (2.5,1.5) -- node[above] {\tiny ${\texttt{SR}}^{-1}$} +(0,0);' + '\n')
...
for r in range(mat, TR):
    if r == mat:
        ...
        f.write(r'\begin{scope}[yshift=' + str(
            ...
            -10 + 9 * ((r+1) % 2)) + 'cm]' + '\n')
        f.write(r'\draw (2.5,1.5) node{\tiny {$K_1$}};' + '\n')
        f.write(r'\draw (1,0) node{\tiny $\oplus$};' + '\n')
        for i in range(0, 4):
            for j in range(0, 4):
                if sccolor[16 * (r - mat-1) + 4 * i + j] != 'gray':
                    f.write(r'  \draw[draw=gray!80,fill=' + sccolor[16 * (r - mat-1) + 4 * i + j] + '] (' + str(
    ...
    if r != TR - 1:
        ...
        for i in range(0, 4):
            for j in range(0, 4):
                f.write(r'  \draw[fill=' + sbcolor[16 * (r - mat ) + 4 * i + j] + '] (' + str(j / 2) + ',' + str(
                    2 - i / 2) + ') rectangle +(0.5,0.5);' + '\n')
    else:
        f.write(r'\draw[thin] (2.5,1.5) -- node[above] {\tiny ${\texttt{SB}}^{-1}$} +(0,0);' + '\n')
        f.write(r'\draw[->] (3,1.5) -- ++(-1,0);' + '\n')
        ...
        for i in range(0, 4):
            for j in range(0, 4):
                f.write(r'  \draw[fill=' + sbcolor[16 * (r - mat) + 4 * i + j] + '] (' + str(j / 2) + ',' + str(
                    2 - i / 2) + ') rectangle +(0.5,0.5);' + '\n')
        f.write(r'\draw(4,0) node {\tiny \tt ciphertext};' + '\n')
        for i in range(0, 4):
            for j in range(0, 4):
                f.write(r'  \draw[fill=gray] (' + str(3+j / 2) + ',' + str(
                    2 - i / 2) + ') rectangle +(0.5,0.5);' + '\n')

        f.write(r'\draw (6,4.5) node{\tiny {$K_0 \oplus K_1$}};' + '\n')
        f.write(r'\draw (4,3) node{\tiny $\oplus$};' + '\n')
        for i in range(0, 4):
            for j in range(0, 4):
                if sbcolor[16 * (r - mat) + 4 * i + j] != 'gray':
                    f.write(r'  \draw[draw=gray!80,fill=' + sbcolor[16 * (r - mat) + 4 * i + j] + '] (' + str(
                        3+j / 2) + ',' + str(5 - i / 2) + ') rectangle +(0.5,0.5);' + '\n')
                else:
                    f.write(r'  \draw[draw=gray!80,pattern = north west lines, pattern color = darkgreen] (' + str(
                        3+j / 2) + ',' + str(5 - i / 2) + ') rectangle +(0.5,0.5);' + '\n')
        f.write(r'\draw[line width=1pt] (3,3.5) rectangle +(2,2);' + '\n')
    f.write(r'	\end{scope}' + '\n' + r'    \end{scope}' + '\n')




f.write(r'\begin{scope}[shift={(-3,6)}]'+'\n')
