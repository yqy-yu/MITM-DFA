...
class Constraints_generator():
    ...
    def genConstraints_of_encrypt(self,r):
        ...
        constr = []
        #constraints for SB
        constr = constr + MITMPreConstraints.equalConstraints(input_sb,input_sc)

        # constraints for SC
        constr = constr + MITMPreConstraints.equalConstraints(SC(input_sc), input_mc)
    ...
    def genConstraints_of_matchinground(self):
        ...
        output_round = Vars_generator.genVars_input_of_SB(self.mat_r+1)
        ...
        constr = constr + [BasicTools.plusTerm(output_round) + ' >= 2']

        # split
        for i in range(4):
            constr = constr + MITMPreConstraints.split_constraint(output_round[i],output_round[4+i],output_round[8+i],output_round[12+i],
                                                                  output1_round[i],output1_round[4+i],output1_round[8+i],output1_round[12+i],
                                                                  output2_round[i],output2_round[4+i],output2_round[8+i],output2_round[12+i])
    ...
    def genConstraints_of_decrypt(self, r):
        ...
        constr = []
        #SB
        constr = constr + MITMPreConstraints.equalConstraints(input1_sb, input1_sc)
        constr = constr + MITMPreConstraints.equalConstraints(input2_sb, input2_sc)

        # SC
        constr = constr + MITMPreConstraints.equalConstraints(SC(input1_sc), input1_mc)
        constr = constr + MITMPreConstraints.equalConstraints(SC(input2_sc), input2_mc)
        ...
        for i in range(4):
            ...
            constr = constr + MITMPreConstraints.MCguess_constraint(input2_mc[i], input2_mc[4 + i], input2_mc[8 + i],
                                                                    input2_mc[12 + i],
                                                                    output2_round[i], output2_round[4 + i], output2_round[8 + i],
                                                                    output2_round[12 + i])
    ...
    def genConstraints_keyguess(self):
        ...
        K11 = Vars_generator.genVars_input1_of_SB(self.TR)
        K12 = Vars_generator.genVars_input2_of_SB(self.TR)
        for i in range(0,16):
            X=[]
            for v in K01:
                X.append(v[i])
    ...
    def genModel(self,filename):
        ...
        fid = open('./midori/TR' + str(self.TR) + '_matr' + str(self.mat_r) + '.lp', 'w')
...
def cmd():
    TR = 4
    mat_r = 2
    name = './midori/TR' + str(TR) + '_matr' + str(mat_r)
    ...
    while counter < 18:
        ...
        if m.Status == 2:
            ...
            for v in m.getVars():
                ...
                if (v.VarName[0:3] == 'IS1') and (v.VarName[len(v.VarName) - 1] == str(mat_r+1)) and (v.x == 1):
                    M1.append(v.VarName)
                if (v.VarName[0:3] == 'IS2') and (v.VarName[len(v.VarName) - 1] == str(mat_r+1)) and (
                        v.x == 1):
                    M2.append(v.VarName)
